...
#define BS_UDB_0_1_1_CONFIG_VAL ((const uint8 CYFAR *)0x48000608u)
...
uint8 CYXDATA * const CYCODE MUX_DAC__addrTable[4] = {
...
/* This is an implementation detail of the AMux. Code that depends on it may be
   incompatible with other versions of PSoC Creator. */
const uint8 CYCODE MUX_DAC__maskTable[4] = {
	0x02u, 0x00u, 
	0x80u, 0x02u, 
};

/*******************************************************************************
* Function Name: MUX_DAC_Set
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void MUX_DAC_Set(uint8 channel)
{
	if (channel < 2)
	{
		channel += channel;
		*MUX_DAC__addrTable[channel] |= MUX_DAC__maskTable[channel];
		channel++;
		*MUX_DAC__addrTable[channel] |= MUX_DAC__maskTable[channel];
	}
}
...
*
*******************************************************************************/
void MUX_DAC_Unset(uint8 channel)
{
	if (channel < 2)
	{
		channel += channel;
		*MUX_DAC__addrTable[channel] &= (uint8)~MUX_DAC__maskTable[channel];
		channel++;
		*MUX_DAC__addrTable[channel] &= (uint8)~MUX_DAC__maskTable[channel];
	}
}
